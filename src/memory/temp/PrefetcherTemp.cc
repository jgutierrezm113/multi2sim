...
void Prefetcher::AccessOnHit  (esim::Frame *esim_frame)
{
	...
	if (cache != NULL) {
		...
		if (prefetch_address > 0)
			PrefetchAction(frame,
				prefetch_address);	
		
		
		/*
		switch (cache->prefetcher_type){
			case Cache::Empty:
				break;
			case Cache::ConstantStrideGlobalHistoryBuffer: break;
			case Cache::DeltaCorrelationGlobalHistoryBuffer: break;
			case Cache::Miss: break;
			case Cache::Always: {
				unsigned int prefetch_address = frame->getAddress() 
